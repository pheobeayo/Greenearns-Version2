...
const convertToWholeNumber = (formattedNumber) => {
  ...
  return (
    <div>
      <div className="flex my-6  text-[#0F160F] items-center">
      <img 
            ...
            />
        {userSeller ? (
            <p className='ml-6 font-bold'>{userSeller.name}</p>
        ) : (
          <p>Unregistered.</p>
        )}
      </div>
          ...
          userProducts.map((info, index) => (
            <div key={index} className="w-[100%] lg:w-[31%] md:w-[31%] rounded-lg border border-bg-ash/35 bg-bg-gray p-4 mt-6">
              <Link to={`/dashboard/market_place/${info.id}`} className='text-[#0F160F]'>
                ...
                />
                <h3 className='font-bold mt-4 lg:text-[20px] md:text-[20px] text-[18px] capitalize'>{info.name}</h3>
